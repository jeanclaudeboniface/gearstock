...
const MOVEMENT_REASONS = {
  ...
  ADJUST: ['AUDIT_CORRECTION', 'COUNT_DISCREPANCY', 'SYSTEM_ERROR_FIX', 'OTHER']
};
...
export default function InventoryMovement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ sparePartId: '', type: 'IN', quantity: '', reason: '', notes: '', unitCost: '', referenceNumber: '' });
      fetchData();
    } catch (err) {
    ...
    <div className="max-w-6xl mx-auto p-4">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">Full audit trail of all inventory changes.</p>
        </div>
        {userRole !== 'VIEWER' && (
          <button 
            onClick={() => setIsModalOpen(true)}
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-indigo-200 transition-all flex items-center"
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase text-right">Qty Change</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase">By</th>
            ...
            <tbody className="divide-y divide-slate-50">
                ...
                <tr key={m.id || m._id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-900">{m.sparePartId?.name}</span>
                      <span className="text-xs text-slate-400 font-mono">{m.sparePartId?.sku}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full inline-block w-fit ${
                        ...
                        m.type === 'OUT' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                      }`}>
                        {m.type} - {m.reason}
                      </span>
                      {m.referenceNumber && <span className="text-[10px] text-slate-400 mt-1 uppercase">Ref: {m.referenceNumber}</span>}
                    </div>
                  </td>
                  <td className={`px-6 py-4 text-right font-mono font-bold ${m.quantity > 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              {movements.length === 0 && !loading && (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-slate-400">No stock history yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Record Inventory Movement">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500 uppercase">Select Spare Part</label>
            <select 
              name="sparePartId" 
            ...
            >
              ...
              {spareParts.map(p => <option key={p.id || p._id} value={p.id || p._id}>{p.name} ({p.sku})</option>)}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
                ...
                {['IN', 'OUT', 'ADJUST'].map(t => (
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
                      form.type === t ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                    }`}
                  >
                    {t}
                  </button>
            ...
            <div className="space-y-1">
              ...
              <input 
                ...
                required 
                min="1" 
                className="w-full border border-slate-200 rounded-xl px-4 py-2.5 outline-none font-mono"
                placeholder="0"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              >
                ...
                {MOVEMENT_REASONS[form.type].map(r => <option key={r} value={r}>{r.replace(/_/g, ' ')}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 uppercase">Ref Number (Optional)</label>
              <input 
          ...
          <div className="pt-4 flex justify-end space-x-3">
            ...
            <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-2.5 rounded-xl font-bold shadow-lg shadow-indigo-100">
              Confirm Record
            </button>
          </div>
        </form>
