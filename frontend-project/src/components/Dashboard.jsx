  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div
          className={`p-8 rounded-3xl border shadow-sm ${stats.lowStockCount > 0 ? "bg-red-50 border-red-100" : "bg-white border-slate-100"}`}
        >
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
            Low Stock Alerts
          </p>
          <div className="flex justify-between items-end">
            <span
              className={`text-4xl font-extrabold ${stats.lowStockCount > 0 ? "text-red-600" : "text-slate-900"}`}
            >
              {stats.lowStockCount}
            </span>
            {stats.lowStockCount > 0 && (
              <Link
                to="/reports"
                className="text-sm font-bold text-red-600 hover:text-red-500 underline decoration-2"
              >
                Restock Now →
              </Link>
            )}
        ...
        <div className="bg-slate-900 p-8 rounded-3xl shadow-xl text-white relative overflow-hidden">
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Link
                to="/inventory/move"
                className="bg-white/10 hover:bg-white/20 p-4 rounded-2xl transition-all group"
              >
              ...
              {userRole === "OWNER" && (
                <Link
                  to="/team"
                  className="bg-indigo-600/50 hover:bg-indigo-600 p-4 rounded-2xl transition-all group"
                >
