...
export default function WorkOrders() {
  ...
  const addPartToOrder = () => {
    setForm({
      ...form,
      parts: [...form.parts, { sparePartId: "", quantity: 1, unitPrice: 0 }],
    });
  };
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm sm:text-base">
            Track garage jobs and part consumption.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {orders.map((order) => (
          <div
            ...
            className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 hover:shadow-md transition-shadow"
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <span
                className={`text-[10px] font-bold px-2 py-1 rounded-full ${getStatusColor(order.status)}`}
              >
                {order.status}
              </span>
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {order.status === "PENDING" &&
                ["OWNER", "MANAGER", "MECHANIC"].includes(userRole) && (
                  <button
                    onClick={() =>
                      updateStatus(order.id || order._id, "IN_PROGRESS")
                    }
                    className="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-2 rounded-lg hover:bg-blue-100"
                  >
                    Start Job
                  </button>
                )}
              {order.status === "IN_PROGRESS" &&
                ["OWNER", "MANAGER", "MECHANIC"].includes(userRole) && (
                  <button
                    ...
                    className="text-xs font-bold text-green-600 bg-green-50 px-3 py-2 rounded-lg hover:bg-green-100"
                  >
              ...
              {order.status === "COMPLETED" &&
                ["OWNER", "MANAGER"].includes(userRole) && (
                  <button
                    onClick={() =>
                      updateStatus(order.id || order._id, "CLOSED")
                    }
                    className="text-xs font-bold text-slate-600 bg-slate-100 px-3 py-2 rounded-lg hover:bg-slate-200"
                  >
                    Archive/Close
                  </button>
                )}
        ...
        {orders.length === 0 && !loading && (
          <div className="col-span-full py-20 text-center">
            <p className="text-slate-400 font-medium">
              No active work orders. Ready for next job?
            </p>
          </div>
        )}
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              required
              className="w-full border border-slate-200 rounded-xl px-4 py-3 sm:py-2.5 outline-none font-bold text-base sm:text-sm"
              ...
              <div key={idx} className="flex gap-2 items-end">
                <div className="flex-1">
                  <select
                    value={p.sparePartId}
                    onChange={(e) =>
                      updatePartItem(idx, "sparePartId", e.target.value)
                    }
                    required
                ...
                <div className="w-24">
                  <input
                    ...
                    value={p.quantity}
                    onChange={(e) =>
                      updatePartItem(idx, "quantity", parseInt(e.target.value))
                    }
                    required
                    min="1"
          ...
          <div className="pt-4 flex justify-end space-x-3">
            ...
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2.5 rounded-xl font-bold shadow-lg shadow-blue-100"
            >
              Create Work Order
            </button>
          </div>
